...
namespace Data.BancoCentral.Api.Infrastructure
{
  public class BancoCentralAPI : IBancoCentralAPI
  {
    private readonly HttpClient _client;
    private readonly string _baseUrl;

    public BancoCentralAPI(HttpClient client, IOptions<BancoCentralOptions> options)
    {
    ...
    public async Task<ExchangeRate> GetExchangeRateAsync(string date)
    {
    ...
    public async Task<ExchangeRates> GetOpeningRateAsync(string date)
    {
      string endpoint = $"odata/CotacaoMoedaDia(moeda=@moeda,dataCotacao=@dataCotacao)?@moeda='USD'&@dataCotacao='{date}'&$format=json";
      return await GetAsync<ExchangeRates>(endpoint);
    }
